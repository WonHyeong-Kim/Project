...
public class ChatDaoImpl implements ChatDaoInter{
	...
	public boolean delete_chatRoom(String chat_id) {
		try {
			int result = inter.delete_chatRoom(chat_id);
			if(result > 0) {
				return true;
			}else {
				return false;
			...
			System.out.println("delete_chatRoom err"+e);
			return false;
		}
	}

	@Override
	public boolean insert_chatMsg(ChatMsgDto dto) {
		try {
			int result = inter.insert_chatMsg(dto);
			if(result > 0) {
				return true;
			}else {
				return false;
			...
			System.out.println("insert_chatMsg err"+e);
			return false;
		}
	}
}